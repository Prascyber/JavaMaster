      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
          <div className="bg-white rounded-lg shadow p-6 border-l-4 border-brand-yellow">
            ...
            <p className="text-2xl font-bold mt-2">{student?.full_name}</p>
          </div>
          <div className="bg-white rounded-lg shadow p-6 border-l-4 border-brand-yellow">
          ...
          <div className="bg-white rounded-lg shadow p-6 border-l-4 border-brand-yellow">
            ...
            <p className="text-lg font-bold mt-2">{student?.college_name}</p>
          </div>
          <div className="bg-white rounded-lg shadow p-6 border-l-4 border-brand-yellow">
            <h3 className="text-gray-600 text-sm font-semibold">Year</h3>
            <p className="text-2xl font-bold mt-2">{student?.year}</p>
          </div>
        </div>
          ...
          <div className="p-6">
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {orders.map((order) => (
                  <div key={order.id} className="bg-gradient-to-br from-brand-black to-gray-900 text-white rounded-lg p-6 hover:shadow-xl transition">
                    <h3 className="text-xl font-bold mb-2">{(order as any).courses?.title}</h3>
                    <p className="text-gray-300 text-sm mb-4">Enrolled on {new Date(order.purchase_date).toLocaleDateString()}</p>
